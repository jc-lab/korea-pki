...
func parseAI(der cryptobyte.String) (pkix.AlgorithmIdentifier, error) {
	...
	if !der.ReadASN1ObjectIdentifier(&ai.Algorithm) {
		return ai, errors.New("x509: malformed x509.OID")
	}
	if der.Empty() {
		return ai, nil
	}
...
func parseExtension(der cryptobyte.String) (pkix.Extension, error) {
	...
	if !der.ReadASN1ObjectIdentifier(&ext.Id) {
		return ext, errors.New("x509: malformed extension x509.OID field")
	}
...
func parseCertificatePoliciesExtension(der cryptobyte.String) ([]x509.OID, error) {
	...
	for !der.Empty() {
		...
		oid := x509.OID{}
		if err := oid.UnmarshalBinary(OIDBytes); err != nil {
			return nil, errors.New("x509: invalid certificate policies")
		}
...
func parseCertificate(der []byte) (*x509.Certificate, error) {
	...
	// we can populate x509.Certificate.Raw, before unwrapping the
	...
	// bytes for x509.Certificate.RawTBSCertificate
		...
		if cert.Version == 3 {
			...
			if present {
				...
				for !extensions.Empty() {
					...
					if seenExts[oidStr] {
						return nil, fmt.Errorf("x509: certificate contains duplicate extension with x509.OID %q", oidStr)
					}
